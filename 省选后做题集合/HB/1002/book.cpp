...
signed main() {
	...
	W = read();
	for(int i = 1; i <= n; ++ i)
		t=std::min(x[i] = a[i].a, W/a[i].w); ans = ans + (i128)t*a[i].v, W -= t*a[i].w, x[i] -= t, y[i] += t; 
	memset(f, 0x80, sizeof(f)); f[0] = 0; lim = m*m + W; 
	for(int i = 1; i <= n; ++ i) {
		for(t = 1; t <= x[i]; t <<= 1) ad1(a[i].w*t, a[i].v*t), x[i] -= t; 
		if(x[i]) ad1(a[i].w*x[i], a[i].v*x[i]); 
	}
	for(int i = 1; i <= n; ++ i) {
		for(t = 1; t <= y[i]; t <<= 1) ad2(a[i].w*t, a[i].v*t), y[i]-=t; 
