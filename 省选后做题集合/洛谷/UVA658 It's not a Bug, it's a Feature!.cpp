...
int dis[1 << 20] , t[205] , b1[205] , b2[205] , f1[205] , f2[205] , pro;
...
void dijkstra(){
    ...
    while(size(q)){
        ...
        for(int i = 1 ; i <= m ; i ++){
            if(((cur.second & b2[i]) == 0) && ((cur.second & b1[i]) == b1[i]))
                // cerr << cur.second << i << " " << (((cur.second | f1[i]) ^ f1[i]) | f2[i]) << " " << dis[((cur.second | f1[i]) ^ f1[i]) | f2[i]] << endl;
...
void solve() {
    ...
    // rep(i ,1 , m){
    //     cerr << t[i] << f1[i] << f2[i] << b1[i] << b2[i] << endl;
    // }
    s = (1 << n) - 1 ;
    dijkstra();
    // write_(dis[0]);
    printf("Product %d\n" , ++pro);
    if(dis[0] == 0x7fffffff){
        vreturn puts("Bugs cannot be fixed.\n");
    }
    vreturn printf("Fastest sequence takes %d seconds.\n\n" , dis[0]);
    end:
    {}
    return ;
}
