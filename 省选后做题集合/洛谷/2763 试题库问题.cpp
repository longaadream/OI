...
int n , k , s , m , t , cls[N6] , inf = 0x7fffffff;

int dis[N5] , head[N5] , cnt = 1 , cur[N6];
struct edge{
    int val , to , nxt;
}e[N5];
...
void solve() {
    ...
    flowe();
    rep(i , 1 , cnt){
        if((e[i].to < e[i ^ 1].to) && (e[i].val) && (e[i].to != n + k + 1) && (e[i].to != n + k + 2) && (e[i ^ 1].to != n + k + 1) && (e[i ^ 1].to != n + k + 2)){
            cerr << e[i].to << " " << e[i ^ 1].to << endl;
            v[e[i ^ 1].to - n].pb(e[i].to);
        }
    }
    // cerr << m << endl;
    int sum = 0;
