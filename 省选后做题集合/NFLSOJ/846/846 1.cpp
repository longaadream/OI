...
struct big {
    ...
    void sc() {
        ...
        len = strlen(pm + 1);
        memset(z, 0, sizeof(z));
        for (int i = 1, j = len; i <= len; i++, j--) z[j] = pm[i] - '0';
    }
    void init() {
        memset(z, 0, sizeof(z));
        len = z[1] = 1;
    ...
    void pt() {
        //      std::cout<<"len "<<len<<"\n";
        for (int i = len; i >= 1; i--) std::cout << z[i];
        puts("");
    }
    void clean() {
        len = 0;
    ...
    bool ji() { return z[1] & 1; }
    bool one() { return len == 1 && z[1] == 1; }
    big operator/(const int &val) const {
        big res;
        res.len = len;
        int las = 0;
        for (int i = len; i >= 1; i--) {
            res.z[i] = (las * 10 + z[i]) / 2;
            las = (las * 10 + z[i]) % 2;
        }
    ...
    big operator+(const int &val) const {
        ...
        res.z[1]++;
        int ps = 1;
        while (res.z[ps] == 10) res.z[ps] = 0, res.z[++ps]++;
        if (res.z[res.len + 1])
            res.len++;
        return res;
    }
    big operator+(const big &b) const {
        ...
        for (int i = 1; i <= res.len; i++) res.z[i] = z[i] + b.z[i];
        for (int ps = 1; ps <= res.len; ps++)
            if (res.z[ps] > 9) {
                res.z[ps] %= 10, res.z[ps + 1]++;
            }
        if (res.z[res.len + 1])
            res.len++;
        return res;
