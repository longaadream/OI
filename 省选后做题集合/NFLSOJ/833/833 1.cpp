...
char fl[1005][1005] , emb[3][3];

map<char,int> jed;
int pm[1005][1005][3];

void solve() {
    jed['J'] = 0 , jed['O'] = 1 , jed['I'] = 2;
    read(n , m);
    ll ans = 0;
    rep(i ,1 , n){
        scanf("%s" , (fl[i] + 1));
    ...
    rep(i ,1 , n){
        rep(j , 1 , m){
            int cnt = 0;
            rep(k , 0 , 3){
                    if(i + dx[k] - 1 > n || j + dy[k] - 1 > m) goto end;
                    if(fl[i + dx[k] - 1][j + dy[k] - 1] != emb[dx[k]][dy[k]]) cnt ++;
            }
            if(!cnt){
                ans ++;
            }
            if(cnt == 1){
                rep(k , 0 , 3){
                    if(fl[i + dx[k] - 1][j + dy[k] - 1] != emb[dx[k]][dy[k]]) {pm[i + dx[k] - 1][j + dy[k] - 1][jed[emb[dx[k]][dy[k]]]]++;}
                }
            }
            end:{}
        }
