...
int cur[N6] , rch[N6] , Q[N6] , id[N6] , ans[N6];

void update(int u) {
    int sz = size(G[u]);
    while(cur[u] < sz && G[u][cur[u]].st <= rch[u]) {
            int to = G[u][cur[u]].to , ed = G[u][cur[u]].ed;
            if(rch[to] < ed) {
                rch[to] = ed;
                update(to);
            }
...
void solve() {
    ...
    sort(id + 1 , id + q + 1, [&](int u , int v){
        return Q[u] < Q[v];
    });
    rep(i ,1 , n){
        if(!size(G[i])) continue;
        sort(G[i].begin() , G[i].end() , [&](node u , node v){
            return u.st < v.st;
        });
        cur[i] = 0;
    }
