...
namespace Rand{
    ...
    int gen(){return gen1()%10000000+1;}
    #undef ull
}

int n , a[20000009] , b[20000009] , f[20000009];
...
void solve() {
    cin>>n>>Rand::k1>>Rand::k2;
    ll ans = 0;
    for(int i=1;i<=n;i++){a[i]=Rand::gen(),b[i]=Rand::gen() + a[i];}
    bucket(a);bucket(b);
    ll tmp = 0;
    for(int i = 1 , t1 = 0 , t2 = 0 , x; i <= n * 3; ++ i , ans ^= tmp){
        for(tmp += a[++t1]; (t1 >= 2) && (x = b[(t2 + 1)] - a[t1] - a[t1 - 1]) > 0; (tmp += x) , t1 -= 2 , ++ t2);
    } 
    write_(ans);
    end:
    {}
