...
void solve() {
    ...
    ll mx = 0;
    rep(i ,0 , m - 1){
            // BUG(*(--upper_bound(a.begin() , a.end() , b[i] + k)));
            if(abs(*(--upper_bound(a.begin() , a.end() , b[i] + k)) - b[i]) <= k)
            _max(mx , (*(--upper_bound(a.begin() , a.end() , b[i] + k)) + b[i]));
            if(abs(*(lower_bound(a.begin() , a.end() , b[i] - k)) - b[i]) <= k) 
            _max(mx , (*(lower_bound(a.begin() , a.end() , b[i] - k)) + b[i]));
        // if(lower_bound(a.begin() , a.end() , b[i] - k) != a.begin()){
        //     _max(mx , (b[i] + *lower_bound(a.begin() , a.end() , b[i] - k)));
        // }
    }
    if(mx == 0){
        vreturn puts("-1");
